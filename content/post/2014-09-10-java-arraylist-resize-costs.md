...
ArrayLists **don't** support guaranteed constant time element insertion because of the following situation:<figure class="wp-caption aligncenter" id="attachment_251" style="width: 350px">

[<img alt="Inserting into an array which doesn't have enough in space." class="wp-image-251 size-full" height="197" src="//codinginthetrenches.com/wp-content/uploads/2014/09/ArrayListOutOfSpace.png" style="background-color: white;" width="350"/>][5]<figcaption class="wp-caption-text">Insert �uh oh� from [Kid Pix][6] here</figcaption></figure> 
...
That's all for today. In short, use pre-sized ArrayLists if it is convenient and/or it happens to be a very performance sensitive application.  Otherwise, in terms of performance it is probably more important to make sure that the application's data layer interactions (SQL queries??) are well written than worrying about pre-sized lists.
 ...
 [9]: https://github.com/msh9/codinginthetrenches-examples/blob/master/ArrayListSizing/src/com/mihughes/examples/arraysize/Arraysize.java</ref>